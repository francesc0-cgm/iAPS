    ...
    struct RootView: BaseView {
        ...
        var body: some View {
            Form {
                ...
                } header: { Text("Intervallo target") }

                Section {
                    Toggle("Skip Bolus screen after carbs", isOn: $state.skipBolusScreenAfterCarbs)
                } header: { Text("Visualizzazione pasti") }

                Section {
                    Toggle("Usa tema colorato (richiede riavvio)", isOn: $state.disco)
                    Toggle("Animated Background", isOn: $state.animatedBackground)
                } header: { Text("Colore e forma") }
            }
            .onAppear(
                perform:
                configureView
            )
            .navigationBarTitle("Personalizza UI")
