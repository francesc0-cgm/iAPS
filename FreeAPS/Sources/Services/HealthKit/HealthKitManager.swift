        ...
        func save(samples: [HKSample]) {
            ...
            healthKitStore.save(samplesToSave) { (success: Bool, error: Error?) -> Void in
                if !success, let error = error {
                    debug(
                        .service,
                        "Failed to store blood glucose in HealthKit Store! Error: " + error.localizedDescription
                    )
                }
            }
        }
        ...
        func save(samples: [HKSample]) {
            ...
            healthKitStore.save(samplesToSave) { (success: Bool, error: Error?) -> Void in
                if !success, let error = error {
                    debug(
                        .service,
                        "Failed to store carb entry in HealthKit Store! Error: " + error.localizedDescription
                    )
                }
            }
        }
